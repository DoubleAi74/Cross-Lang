...
import { updatePreferences } from "@/lib/lists/service";
...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const wordList = await updatePreferences(id, session.user.id, body);
    const serialized = serializeList(wordList);

    return NextResponse.json({
      showRomanization: serialized.showRomanization,
    });
  } catch (error) {
